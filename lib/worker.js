...
function startWorker() {
  amqpConn.createChannel(function(err, ch) {
    ...
    function processMsg(msg) {
      createPurchaseOrderXML(msg, function(ok) {
        try {
          if (ok)
            ch.ack(msg);
          else
...
function createPurchaseOrderXML(msg, cb) {
  try {
    if (msg != null) {
      console.log('*********************');
      ...
      for (x=0; x < payld.purchase_order_items.length; x++) {
        ...
        poLineAdd.ele('Rate', parseFloat(payld.purchase_order_items[x].unit_price));
      }
