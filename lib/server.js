...
function startWorker() {
  amqpConn.createChannel(function(err, ch) {
    ...
    ch.assertQueue("xml-queue", { durable: true }, function(err, _ok) {
      ...
      console.log("XML Queue Worker is started");
    });
...
function work(msg, cb) {
  console.log("Got XML Message", msg.content.toString());
...
var server = http.createServer(function(req, res) {
    ...
    if (req.method == "POST" && req.url == "/webhook") {
    ...
    } else if (req.method == 'GET' && (req.url == '/admin' || req.url == '/edituser')) {
       ...
       console.log('request ', req.url);
        ...
        if (filePath != './') {
            ...
            } else if (filePath == './edituser') {
                filePath = './views/pages/edit.ejs';
            }

            console.log("FilePath has been changed to: " + filePath);
