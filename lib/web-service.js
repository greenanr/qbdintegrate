...
var companyFile = 'C:\\TestBusiness.qbw';
/**
...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate = function(args, callback) {
    ...
    //Get Username and then check password
    db.getUser(args.strUserName, function(err, result) {
        if (result != null) {
            console.log("result: " + result.toString());
            username = result.qbdusername;
            console.log("Username: " + username);
            password = result.qbdpassword;
            console.log("Password: " + password);
            authReturn[0] = result.uuid;
            console.log("UUID: " + authReturn[0]);
            //companyFile = result.qbdcompanyfile;
        ...
        //console.log("args uname: " + args.strUserName.trim());
        //console.log("args pwd: " + args.strPassword.trim());

        if (amqpLib) {
            console.log("[SOAP Service] Queue connection exists");
            //amqpLib = connection;
        } else {
            console.log("[SOAP Service] Needs to be created");
            amqp.connect(process.env.CLOUDAMQP_URL + "?heartbeat=60", function(err, conn) {
            
                if (err) {
                    console.error("[AMQP SOAP Service 1] ", err.message);
                }
                conn.on("error", function(err) {
                    if (err.message !== "Connection closing") {
                        console.error("[AMQP SOAP Service 2] connection error", err.message);
                    }
                console.log("[AMQP SOAP Service] connected");
                amqpLib = conn;    
                });
            });
        }
        ...
        if (args.strUserName.trim() == username && args.strPassword.trim() == password) {
            ...
            if ((typeof qbXMLHandler.fetchRequests === "function")) {
                qbXMLHandler.fetchRequests(amqpLib, function(err, requests) {
                    requestQueue = requests;
                    if (err || requestQueue.length === 0) {
                        console.log("return none");
                        authReturn[1] = 'NONE';
                    } else {
                        console.log("return companyfile");
